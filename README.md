...
原来只有 8×16 一套字体，每屏只能放 4 行 × 16 字符，列表里只有呼号、详情里只有 2 行正文。
现在增加 6×8 小字体（`font6x8.h`，由 `tools/gen_font.py --small` 生成，**固件与模拟器共用**），
排版变为「8×16 标题 + 1px 分隔线 + 6 行 6×8」，每行 21 字符：

| 屏幕 | 改进 |
|---|---|
| 开机 | 新增开机画面（机型 / MCU / LCD / BK4802P / 固件版本） |
| 待机 | 新增 `NEW n` 未读徽标；底部显示最近一条的来源、类型与正文摘要 |
| 收件箱 | 6 行列表，每行 = 呼号 + 类型 + **正文摘要** + 未读 `*`；选中行**反显**；标题栏显示 `n/N` 与未读数 |
| 详情 | 正文从 2 行增加到 **5 行**；页脚显示页码，含中继时直接给出 `RELAY <路径>` |
| 删除 | 由直接删除改为**反显弹窗二次确认**（`OK=YES / BACK=CANCEL`） |

驱动侧新增 `lcd_draw_char6x8()` / `lcd_draw_string6x8()` / `lcd_fill_rect()`（反显用），
仍在 `lcd_st7567.c` 里，真机与模拟器同一份代码。居中/右对齐都吸附到字符栅格（8 或 6 的整数倍），
否则字会落在半个字符上、点阵发糊。

用真实 5km 日志回放，模拟器读屏结果：

```text
== standby ==                        == inbox ==
y0   |  BBCALL APRS|                 y0   |INBOX 13/13|
y16  |144.640 MHz|                   y16  |BI4BKX C 3116.01N 12*|
y24  |RX 13          MSG 13|         y24  |BD4BE  C 3111.28N 12*|
y32  |NEW 13|                        y32  |BG4AIZ C 3137.15N 12*|
y40  |---------------------|         y40  |BH4FSK C 3037.90N 12*|
y48  |BD4BE  MIC-E|                 y48  |BD4SDX P 3054.31N 12*|
y56  |3111.28N 12125.77E|            y56  |BD4BE  C 3111.28N 12*|  <-反显(选中)

== detail ==                         == confirm ==
y0   |MIC-E BD4BE|                   y24  | DELETE THIS MESSAGE?|  <-反显
y16  |3111.28N 12125.77E|            y32  | OK=YES   BACK=CANCEL|  <-反显
y24  |M0: Off Duty 2km/h|
y32  |223 `"5f} $|
y56  |1/1 RELAY BH4ERY*,WID|
```
