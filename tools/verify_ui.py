...
SPEC = {
    ...
    "inbox": [
        ...
        R(4, 0, "BG5BLB", 12, 1, 1),
        R(80, 0, "*1/4", 12, 1, 1),          # 顶栏右侧：* = 未读 + i/N 右对齐
    ...
    # 未读逻辑：进收件箱按一次 ● -> 第 1 条标记已读并前进到第 2 条；顶栏标记从 '*' 变 ' '（未读数 4->3）
    "inbox_read": [
        ("fill", 0, 0, 127, 11, 1),
        R(4, 0, "BG5BLH", 12, 1, 1),
        R(80, 0, " 2/4", 12, 1, 1),
        RC(0, 127, 16, "中继信号 正常", 12, 1, 0),
        R(4, 40, "14:25:13", 12, 1, 0),
        R(76, 40, "RSSI -88", 12, 1, 0),
    ],
}
...
def main():
    if len(sys.argv) < 3 or sys.argv[2] not in SPEC:
        ...
        print("inbox2 需加 --screen idle --keys \"3,2\"；inbox_read 需加 --keys \"3,3\"")
        sys.exit(2)
