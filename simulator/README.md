...
| 字体 | 尺寸 | 容量 | 用途 |
|---|---|---|---|
| `font8x16.h` | 8×16 | 16 字符/行 | 屏幕标题 |
| `font6x8.h` | 6×8 | **21 字符/行** | 列表、正文、状态 |

版面统一为「8×16 标题 + 1px 分隔线 + 6 行 6×8」：`y=0` 标题、`y=15` 分隔线、`y=16/24/32/40/48/56` 六行。

| 屏幕 | 内容 |
|---|---|
| `boot` | 开机画面：机型 / MCU / LCD / 芯片 / 固件版本 |
| `standby` | 频率、`RX`/`MSG` 计数、`NEW n` 未读徽标、最近一条的来源/类型/摘要 |
| `inbox` | 6 行列表：呼号 + 类型 + **正文摘要** + 未读 `*`；选中行**反显**；标题栏显示 `n/N` 与未读数 |
| `detail` | 标题（类型+呼号）+ **5 行正文**；页脚显示 `页/总页`，含中继时显示 `RELAY <路径>`，否则 `FIX`/`REP` |
| `confirm` | 删除确认：反显弹窗 `DELETE THIS MESSAGE?` / `OK=YES BACK=CANCEL` |
| `pattern` | 棋盘格 + 两套字体的全字符样张（换字体/换屏后一眼看出缺字） |

行为细节：

- 未读模型：新入箱条目记未读，按 Enter 打开后置为已读；`NEW` 徽标随之更新；
- 删除两步走：Delete 弹确认，OK 才真删（同时把该条标记已读），BACK 取消；
- 收件箱容量 24，满时丢弃最旧一条；列表滚动窗口跟随选中项；
