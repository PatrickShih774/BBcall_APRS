...
- 条目类型：`M` 消息、`P` 位置、`C` Mic-E、`X` 其它；列表右侧显示时间（日志有 `[T=..ms]` 时）
  或会话序号 `#nn`（日志没有时间戳时，避免显示假的 `0s`）；
- 详情页显示呼号 + 折行正文，最多 6 行、每页 2 行，右下角标注 `FIX`（1/2bit 纠错恢复）
  或 `REP`（参考帧恢复）或 `RELAY`（该帧含中继路径）；
- 删除会即时重排列表与滚动窗口；收件箱容量 24，满时丢弃最旧一条。
