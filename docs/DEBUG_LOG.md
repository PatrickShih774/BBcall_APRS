...
```text
[I2C] no response: scl=1 sda=1 ack=0 (1=high; ack=1 means chip answers)
```

实现：`bk4802_bus_probe()`（`bk4802.c`）把 SCL/SDA 释放成输入 + 上拉，读空闲电平，再试一次"写地址"看从机 ACK；
探测自身产生的 NACK 不计入 `I2CE`（否则每探一次都涨）。

| 组合 | 结论 | 下一步 |
|---|---|---|
| `scl=0` 或 `sda=0` | 总线被拉死：短路到 GND、走线被焊锡短接、或从机把线拉低不放 | 用万用表量 PA9/PA10 对地阻抗；查焊点 |
