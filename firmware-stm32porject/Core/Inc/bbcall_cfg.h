...
/* ---------- RTC 时钟源选择 ----------
 * 0 = 自动：LSE(32.768k 晶振) -> HSE/128(主板 8MHz) -> LSI（兼容没焊 32.768k 晶振的板子）；
 * 1 = 只用 LSE（板上确定有 32.768k 晶振时用；起振失败会明确报错，不会静默换源）；
 * 2 = 只用 HSE/128；3 = 只用 LSI。
 * 上电串口会打印 [RTC] cfg=<档> src=<实际> lse=<起振ms>。 */
#define BBCALL_RTC_CLK_SRC 1
/* RTC 走时校准（ppm）：正数 = 实测走快了，需要把分频比调大来减慢。
 * 测法：跑满 24 小时与 PC 对一次，例：24h 快了 45 秒 -> 45/86400 = 520ppm，这里填 520。
 * 分辨率：LSE 一档 30.5ppm（2.6 秒/天），HSE/128 一档 16ppm（1.4 秒/天）。
 * 串口也可用 TRIM=±ppm 改（存备份寄存器、复位不丢），本宏只是"全新备份域"时的初值。 */
#define BBCALL_RTC_TRIM_PPM 0
